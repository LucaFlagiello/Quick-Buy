    ...
    <header className={component === 'mobile-menu' ? '' : "flex items-center justify-between py-4 m-auto max-w-[1140px] lg:max-w-[930px] md:hidden sm:hidden"}>
      <div className='flex items-center gap-x-24 sm:gap-x-0 md:gap-x-0'>
        ...
        <ul className={component === 'mobile-menu' ? 'gap-y-3 font-Poppins  text-[1.1rem] w-full' : 'flex gap-x-8 font-Poppins font-medium text-secondary-color text-[16px]'}>
          <Link to={'/'} onClick={closeLinks}><li className={component === 'mobile-menu' ? 'hover:bg-[#f3f3f3] pt-[14px] pb-3 pl-4 border-b' : `hover:text-${theme} cursor-pointer transition duration-[.3s] ease`}>Home</li></Link>
          <Link to={'/Shop/:page'} onClick={closeLinks}><li className={component === 'mobile-menu' ? 'hover:bg-[#f3f3f3] pt-[10px] pb-3 pl-4 border-b' : `hover:text-${theme} cursor-pointer transition duration-[.3s] ease`}>Shop</li></Link>
