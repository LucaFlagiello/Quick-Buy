...
export default function NewArrivalsProduct({ img, model, originalPrice, newPrice, reviewsNum, theme, productPreviewStatus, dispatch, showPreview, productTag, isProductsList }) {
  ...
  return (
    <div className={isProductsList ? 'flex sm:flex-col' : "group"}>
      <div className={isProductsList ? 'relative w-[300px] bg-[#f3f3f3] rounded-t-md py-6 sm:w-full' :"relative bg-[#f3f3f3] rounded-t-md py-6"}>
        <img src={img} alt="product-img" className={isProductsList ? "m-auto object-contain h-[220px]" :  "object-contain w-full h-[140px]"}/>
        ...
        <div className="flex items-center mt-2 gap-x-2">
          ...
          <div className={isProductsList ? "" : "hidden"}>
            ...
            <div className="flex mt-4 gap-x-4 2xs:grid 2xs:gap-y-3 2xs:max-w-[175px]">
              <button onClick={() => addNewProduct('cart')} className={`flex items-center gap-x-3 bg-${theme} text-white border border-${theme} text-Poppins font-medium py-[10px] px-6 rounded-[4px] text-[15px] hover:bg-transparent hover:text-${theme} transition duration-[.5s] ease group`}>
              ...
              <button onClick={() => addNewProduct('list')} className={`flex items-center gap-x-3 bg-white text-${theme} border border-${theme} text-Poppins font-medium py-[10px] px-8 rounded-[4px] text-[15px] hover:bg-${theme} hover:text-white transition duration-[.5s] ease group`}>
